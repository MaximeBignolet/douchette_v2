	...
	try {
		...
		const handleResult = (result?: Result, error?: Exception) => {
			if (result) {
				...
				emit(`scan`, value);
				showToast(`Scan: ${value}`, `success`);
			}
			else if (error && error.name !== `NotFoundException`) {
				// Surface unexpected decoding errors for easier debugging.
				console.error(error);
				showToast(`Erreur scan: ${error.message ?? error.name}`, `error`);
			}
		};
		...
		console.error(error);
		showToast(`Caméra indisponible: ${(error as Error).message ?? `inconnue`}`, `error`);
		stopScan();
	}
}
...
<template>
	<div class="relative flex flex-col gap-4">
		...
		>
			<span
				class="inline-flex items-center justify-center rounded-full px-4 py-2 text-xs font-semibold text-white shadow-lg"
